...
void setup(){
  ...
  debug_sid_array = (uint8_t *) calloc(sensor_nval, sizeof(uint8_t));
  ...
  data_value_array = (uint8_t *) calloc(sensor_nval, data_bytes);
  ...
  for (int iS = 0; iS<sensor_nval; iS++){
    ...
    Serial.print("\t");Serial.print(sensor_sid_array[iS]); 
  }
  Serial.print("\r\n");
  ...
  for (int iS = 0; iS<debug_nval; iS++){
    ...
    Serial.print("\t");Serial.print(debug_sid_array[iS]); 
  }
  Serial.print("\r\n");
...
void loop(){
  ...
  for (int iS = 0; iS<sensor_nval; iS++){
    ...
    Wire.write(sensor_sid_array[iS]);
    Wire.endTransmission();
    ...
    Serial.print("Sensor SID: "); Serial.print(sensor_sid_array[iS]); Serial.print(" Datum "); //Serial.print((char*)return_byte_array);
    switch(data_bytes){
      case 4:
        memcpy(&dummy_float, &data_value_array[iS*data_bytes], data_bytes);
        Serial.println(dummy_float);
        break;
      case 2:
